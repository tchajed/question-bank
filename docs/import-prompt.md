...
13. **After creating files**, validate them with `question-bank validate -b <bank>`. This reports parse errors and warnings. Then check that the PDF builds with `question-bank render -b <bank> --metadata --solutions <exam.toml>`.
